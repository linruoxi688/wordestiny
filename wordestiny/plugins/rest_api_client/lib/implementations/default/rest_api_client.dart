...
class RestApiClient extends DioMixin implements IRestApiClient {
  ...
  Future<IRestApiClient> init() async {
    ...
    if (jwt != null) {
      if (restApiClientOptions.header)
        _addOrUpdateHeader(
            key: RestApiClientKeys.authorization, value: 'Bearer $jwt');
      if (restApiClientOptions.parameter) {
        _configureParameterToken(jwt);
      }
    }
  ...
  Future<bool> isAuthorized() async {
    final containsAuthorizationHeader = restApiClientOptions.header
  ...
  Future _configureParameterToken(String jwt) async {
    interceptors.add(
      InterceptorsWrapper(
        onRequest: (RequestOptions options, handler) {
          if (options.method == "GET")
            options.queryParameters
                .addAll({restApiClientOptions.jwtParameterName: jwt});
          else
            options.data.addAll({restApiClientOptions.jwtParameterName: jwt});
  ...
  bool get _usesAutorization =>
      options.headers.containsKey(RestApiClientKeys.authorization) ||
      restApiClientOptions.parameter;
  ...
  Future refreshTokenCallback(DioError error) async {
    if (restApiClientOptions.resolveJwt != null &&
      ...
      final response = await Dio(BaseOptions()
        ...
        options: Options(method: restApiClientOptions.method),
        data: restApiClientOptions.method == "GET"
            ? {
                restApiClientOptions.refreshTokenParameterName:
                    await _getRefreshToken()
              }
            : null,
        queryParameters: restApiClientOptions.method != "GET"
            ? {
                restApiClientOptions.refreshTokenParameterName:
                    await _getRefreshToken()
              }
      ...
      if (response.statusCode == HttpStatus.unauthorized ||
          (restApiClientOptions.parameter &&
              restApiClientOptions.resolveCode!(response) ==
                  restApiClientOptions.unauthorizedCode)) {
        interceptors.requestLock.unlock();
        interceptors.responseLock.unlock();
        exceptionOptions.reset();
        restApiClientOptions.refreshFallback!();
          ...
          if (_usesAutorization) {
            if (restApiClientOptions.parameter &&
                restApiClientOptions.resolveCode!(response) ==
                    restApiClientOptions.unauthorizedCode) {
              try {
                return handler.resolve(await refreshTokenCallback(
